...
class Controllermodulepavbloglatest extends Controller {
	...
	public function index($setting) {
		...
		$this->mdata['newstitle'] = 'Tin tức';
		$this->mdata['commenttitle'] = 'Phản hồi khách hàng';
		$this->mdata['blogtitle'] = 'Blog';
		$data = array(
			'sort'  => 'b.`created`',
			'order' => 'DESC',
			'start' => 0,
		...
		if( isset( $setting['description'][$this->config->get('config_language_id')] ) ) {	
			...
			$this->mdata['message'] = '';
				...
				// Blog
		$data['sort'] = 'b.`hits`';
		$bloghit = $this->model_pavblog_blog->getListBlogs( $data );
		foreach( $bloghit as $key => $blog ){
			if( $bloghit[$key]['image'] ){	
				$bloghit[$key]['thumb'] = $this->model_tool_image->resize($blog['image'], $setting['width'], $setting['height'] );
			}else {
				$bloghit[$key]['thumb'] = '';
			}					
			
			$bloghit[$key]['description'] = html_entity_decode($blog['description'], ENT_QUOTES, 'UTF-8');
			$bloghit[$key]['author'] = isset($users[$blog['user_id']])?$users[$blog['user_id']]:$this->language->get('text_none_author');
			$bloghit[$key]['category_link'] =  $this->url->link( 'pavblog/category', "path=".$blog['category_id'] );
			$bloghit[$key]['comment_count'] =  10;
			$bloghit[$key]['link'] =  $this->url->link( 'pavblog/blog','blog_id='.$blog['blog_id'] );
		}
		$this->mdata['bloghit'] = $bloghit;
